...
bool Secure::CheckPeerIP(struct sockaddr_in *sap, bool ota) {
  ...
  // Use a LWIP function to translate IP into ARP, returns an index into the table (or -1)
  ix = etharp_find_addr(NULL, (ip4_addr_t *)&sap->sin_addr.s_addr, &eap, &iap);
  ESP_LOGD(secure_tag, "%s etharp_find_addr -> %d", __FUNCTION__, ix);
...
void Secure::AddDevice(const char *mac, bool ota) {
  ...
  uint8_t *p = (uint8_t*) &secure_tbl[ndevices].mac;
  for (int i=0; i<6; i++) {
    int x;
    sscanf(mac + i * 3, "%02x", &x);
    p[i] = x;
  }
  secure_tbl[ndevices].ip.addr = 0;
  secure_tbl[ndevices].ota = ota;
  ndevices++;
}
...
void Secure::AddDevice(in_addr_t ip, bool ota) {
  ...
  bzero(&secure_tbl[ndevices].mac, sizeof(struct eth_addr));
...
void Secure::TlsServerStartTask() {
  int server_port = config->getJSONServerPort();
...
int Secure::TlsSetupAcme() {
  ...
  if (config->enforceAuthentication() & config->CONFIG_VERIFY_CLIENT_CERTIFICATE) {
...
int Secure::TlsSetupLocal() {
  ...
  if (config->getMyCert()) {		// Read certificate from file
    sprintf(path, config->base_fmt, config->getMyCert());
  ...
  if (config->getCaCert()) {
    sprintf(path, config->base_fmt, config->getCaCert());
    ret = mbedtls_x509_crt_parse_file(srvcert, path);
  } else {
#ifdef HARDCODED_CERTIFICATES
  ...
  if (config->getMyKey()) {
    sprintf(path, config->base_fmt, config->getMyKey());
...
void Secure::TlsTaskLoop(void *ptr) {
  ...
  sprintf(port, "%d", config->getJSONServerPort());
  ...
  if ((ret = mbedtls_net_bind(&listen_fd, NULL, port, MBEDTLS_NET_PROTO_TCP)) != 0) {
    ...
    ESP_LOGE(tls_tag, "Failed to bind to https://localhost:%d, error %d (%s)",
      config->getJSONServerPort(), ret, error_buf);
    TlsShutdown();
    tsk = tlsTask;
    tlsTask = 0;
    vTaskDelete(tsk);
  ...
  ESP_LOGI(tls_tag, "Bind on https://localhost:%d ok", config->getJSONServerPort());
  ...
  while (1) {
    ...
    // Ok, interpret it now
#ifdef USE_PEERS
    char *q = peers->HandleQueryAuthenticated(p, caller);
#else
    char *q = 0;
    // HandleQueryAuthenticated(p, caller);
#endif
  ...
  // Returning crashes the FreeRTOS, so need to delete the task
  tsk = tlsTask;
  tlsTask = 0;
  vTaskDelete(tsk);
}
